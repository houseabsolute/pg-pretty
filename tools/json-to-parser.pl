...
has _structs_to_wrap => (
    ...
    sub _build_enums ($self) {
            my $enums
                = decode_json(
                path( $self->from, qw( srcdata enum_defs.json ) )
                    ->slurp_utf8 );
    ...
    sub _build_structs ($self) {
            my $structs
                = decode_json(
                path( $self->from, qw( srcdata struct_defs.json ) )
                    ->slurp_utf8 );
            return {
            map      { $_->%* }
                map  { $structs->{$_} }
    ...
    sub _build_node_types ($self) {
            return [
                ...
                grep { !/\A[a-z]/ } decode_json(
                    path( $self->from, qw( srcdata nodetypes.json ) )
                        ->slurp_utf8
                )->@*
            ];
    }
...
EOF
    ...
    sub run ($self) {
            ...
            for my $name ( sort keys $structs->%* ) {
                $code
                    .= $self->_one_struct( $name, $self->_structs->{$name} );
            }
